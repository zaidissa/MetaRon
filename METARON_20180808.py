...
import csv, re, sys, socket, string, random, os, subprocess, time, shutil,
errno, os.path, argparse
...
##    parser.add_argument("-h", "--help", help= "	Prints help message")
    args = parser.parse_args()
##    print parser.parse_args()          ## if arguments are not correct then print help/usage
...
## if process is "op"then specific parameters will be available
        ...
        if args.igp:
            gene_file = args.igp    ## (THIS MIGHT WORK FOR NEW VERSION)            
...
##                print 'SCAF FILE IS',os.path.exists(scaf_file)
...
##                print 'OK4'
        ...
        sys.exit(1)
##    global process_selector, sample_name,scaf_file, gene_file, output_dir, out_dir, read_type, read_length, raw_read1, raw_read2

##FETCHING FILE NAME
##        path_2_file = os.path.basename(gene_pred_file)
##        title_1 = path_2_file.split(".")
##        folder_title = title_1[0]
    folder_title = sample_name
    print 'Sample name:    ', folder_title


##        scaftig_file_path = open (scaf_file, 'r')     ## added to assembly process function
...
##................................................................................................
    if process_selector == "ago":
        ...
        gene_name, strand, gene_st, gene_end,scaf_name_len_dict = seq_info(gene_name_scaf, strand, gene_st, gene_end, file_name, scaftig_name, gene_name, scaf_name4dict)
...
##        gene_name, strand, gene_st, gene_end,scaf_name_len_dict = seq_info2(gene_name_scaf, pathname, strand, gene_st, gene_end, file_name, scaftig_name, gene_name, scaf_name4dict)
        upstream_seq_len, scaftig_name_new, gene_name_new, strand_new, gene_st_new, gene_end_new, scaf_name4dict_new, upstream_st_pos, upstream_en_pos = upstream_coordinates_extraction(scaftig_name, gene_name, strand, gene_st, gene_end, scaf_name4dict)
        leng_scaftig = scaftig_length (scaf_name4dict, scaf_name_len_dict)
        downstream_seq_len, downstream_st_pos, downstream_en_pos = downstream_coordinate_region(scaftig_name, gene_st, gene_end, leng_scaftig, scaftig_name_new, gene_end_new)
        saving_data(scaftig_name_new, gene_name_new, strand_new, gene_st_new, gene_end_new, upstream_seq_len, downstream_seq_len)
...
##        
##    elif process_selector == "ago":
##        assembly_process()
##        header_replacer_F(scaftig_file_unprocessed,scaftig_length_file, sample_name)
##        gene_name_scaf, strand, gene_st, gene_end, file_name, scaftig_name, gene_name, scaf_name4dict = data_extraction(gene_pred_file, gene_pred_tool)
##        gene_name, strand, gene_st, gene_end,scaf_name_len_dict = seq_info(scaftig_file_path, gene_name_scaf, strand, gene_st, gene_end, file_name, scaftig_name, gene_name, scaf_name4dict)
##        upstream_seq_len, scaftig_name_new, gene_name_new, strand_new, gene_st_new, gene_end_new, scaf_name4dict_new, upstream_st_pos, upstream_en_pos = upstream_coordinates_extraction(scaftig_name, gene_name, strand, gene_st, gene_end, scaf_name4dict)
##        leng_scaftig = scaftig_length (scaf_name4dict, scaf_name_len_dict)
##        downstream_seq_len, downstream_st_pos, downstream_en_pos = downstream_coordinate_region(scaftig_name, gene_st, gene_end, leng_scaftig, scaftig_name_new, gene_end_new)
##        saving_data(scaftig_name_new, gene_name_new, strand_new, gene_st_new, gene_end_new, upstream_seq_len, downstream_seq_len)
...
##    else:
##        """
##    "-n", "--sample", help= "Sample name without any dot, underscore or dash")
##    "-p", "--process", help= "1. ago: assembly gene prediction and operon prediciton 2. op: operon prediction only. If 'ago', please provide the following parameters: -n,-rl,-rt,[-pe1,pe2|-pm],")
##    "-rt", "--read_type", help= "Enter read type. 'merge' if the reads are paired-end in two file. 'paired' if the reads are paired-end in one file.")
##    "-rl", "--read_length", help="Enter 'l'if read length is longer than 128 bases and 'r' if read length is shorter than 128 bases")
##    "-pe1", "--paired_1", help= "Enter enter paired read file 1")
##    "-pe2", "--paired_2", help= "Enter enter paired read file 2")
...
##    "-j", "--isc", help= "Select the file containing all scaftigs")
##    "-t", "--tool", help= "Enter 1 for MetaGeneMark, 2 for Prodigal")
##    "-o", "--output", help= "Enter output destination folder")
##    """
##    sys.exit(1) 
##................................................................................................
...
##    print "Scaftig file saved in ", scaftig_file_unprocessed
    scaftig_length_file = output_dir+"/"+sample_name+"_len.txt"
##    pathname = os.path.abspath(scaftig_file_unprocessed)
##    print 'Pathname:   ', pathname
##    print 'calculation of scaftig length completed'
##    global scaftig_file_unprocessed, scaftig_length_file
##    print scaftig_file_unprocessed
##    print scaftig_length_file
    print 'assembly completed'
##    global scaftig_file_unprocessed, scaftig_length_file, sample_name
    
    return (scaftig_file_unprocessed, scaftig_length_file, sample_name)
    
## this must return scaffold file which will be used as input for header_replacer_F
##................................................................................................
...
##    print output_dir
##    print "SCAFTIG_FILE ", scaftig_file
##    print sample_name
    cmd7 = "prodigal -i "+scaftig_file+" -d "+sample_name+"_seq -f gff -p meta -s "+sample_name+"potential_genes -o "+output_dir+"/"+sample_name+" > "+sample_name+"_"+"gene_prediction.log"
    cmds = [cmd7]
##    for cmd in cmds:
##        subprocess.call(cmd, shell = True)
...
##    global gene_name_scaf, strand, gene_st, gene_end
...
##    print 'gene_name_scaf', len(gene_name_scaf)
##    print gene_name_scaf[:10]
    
    
                                            #####################   metagenemark   #####################    
    if gene_pred_tool == '1':
        print "METAGENEMARK"
##        global file_name, scaftig_name, gene_name, scaf_name4dict        
...
##        print 'scaf_name', len(scaftig_name)
...
##gene_name, strand, gene_st, gene_end,scaf_name_len_dict = seq_info(scaftig_file, gene_name_scaf, strand, gene_st, gene_end, file_name, scaftig_name, gene_name, scaf_name4dict)
...
def seq_info(gene_name_scaf, strand, gene_st, gene_end, file_name, scaftig_name, gene_name, scaf_name4dict):
    ...
    print 'pathanme111', pathname
##    global pathname
##    with open(output_dir+"/"+sample_name+"_scaf.fa", mode='r', buffering=-1) as scaftig_file1:
    with open(pathname, mode='r', buffering=-1) as scaftig_file1:

##    scaftig_file1 = open(scaftig_file, 'r')
        scaf_line = []
        scaf_len_only = []
...
##def seq_info2(gene_name_scaf, strand, gene_st, gene_end, file_name, scaftig_name, gene_name, scaf_name4dict, pathname):
##    print 'Extracting sequence information 2...'
##
####    print 'seqinfo2 SCAF FILE IS',pathname
##
####    cmd4 = "cp "+ output_dir+"/"+sample_name+".assembly/scaffold.fa "+output_dir+"/"+sample_name+".scaffold.fa"         #copy and rename scaffold file to output directory
####    cmds = [cmd2]
####    for cmd in cmds:
...
##            scaf_name_len_dict[key] = int(scaf_name_len_dict[key])
##
##    ##    print 'DEF_2 \nscaf_name', len(scaftig_name)
##    ##    print 'gene_name', len(gene_name)
##    ##    print 'strand', len(strand)
##    ##    print 'gene_st', len(gene_st)
##    ##    print 'gene_end', len(gene_end)
##    ##    print 'scaf_name4dict', len(scaf_name4dict)
##        
##        if len(scaftig_name) == len(gene_name) & len(gene_name) == len(strand) & len(strand) == len(gene_st) & len(gene_st) == len(gene_end)& len(gene_end) == len(scaf_name4dict):
##            pass
##            print "Saving..." 
...
##            print 'Number of entries for each gene are not equal, please correct the data and run again'
##            exit()
##        print 'sequence information extraction 2 completed'
##    return gene_name, strand, gene_st, gene_end,scaf_name_len_dict
...
def upstream_coordinates_extraction(scaftig_name, gene_name, strand, gene_st, gene_end, scaf_name4dict):
    ...
    for i in xrange(len(scaftig_name)):
        ...
        if i == len(scaftig_name)-1 and scaftig_name[i] == scaftig_name[i-1]:
            ...
            gene_end_new.append(gene_end[i])
            scaf_name4dict_new.append(scaf_name4dict[i])
##    print 'upstream_seq_len',upstream_seq_len[:10]
    upstream_seq_len = map(int, upstream_seq_len)
##    print 'upstream_seq_len',upstream_seq_len[:10]
##    print 'gene_st_new', gene_st_new[:10]
    gene_st_new = map(int, gene_st_new)
##    print 'gene_st_new', type(gene_st_new), gene_st_new[:10]
    ...
    for i in xrange(len(scaftig_name_new)):
        if i < len(scaftig_name_new)-1:
            ...
            if scaftig_name_new[i] == scaftig_name_new[i-1]:
                ...
                upstream_st_pos.append(up_st)
                upstream_en_pos.append(up_en)
##    print 'Upstream region/coordinate extraction completed.'    
...
##    print leng_scaftig[:10]
##    print 'scaftig length extraction done'
    return leng_scaftig
    ...
    for i in xrange(len(scaftig_name_new)):
        if i < len(scaftig_name_new)-1:
            ...
            if scaftig_name_new[i] == scaftig_name_new[i-1]:
                ...
                downstream_st_pos.append(ds_st)
                downstream_en_pos.append(ds_en)
##    print 'Downstream cregion/oordinate extraction finished.'
...
##    print "Data Manipulation..."
    ##scaftig_name_new, gene_name_new, strand_new, gene_st_new, gene_end_new, upstream_seq_len, upstream_st_pos, upstream_en_pos, downstream_seq_len, downstream_st_pos,downstream_en_pos
    newUPS_st_pos = []
    newUPS_en_pos = []
    newDSS_st_pos = []
    newDSS_en_pos = []
...
##                new_strand.append(strand_new[i])
...
##    print 'newDSS_en_pos\n',newDSS_en_pos[:10], '\n', len(newDSS_en_pos)
####    print 'new_strand\n',new_strand, '\n', len(new_strand)
##    print 'scaf_name4cord_UPS\n',scaf_name4cord_UPS[:10], '\n', len(scaf_name4cord_UPS)
##    print 'gene_name4cord_UPS\n',gene_name4cord_UPS[:10], '\n', len(gene_name4cord_UPS)
##    print 'gene_name4cord_DSS\n',gene_name4cord_DSS[:10], '\n', len(gene_name4cord_DSS)
##    print 'scaf_name4cord_DSS\n',scaf_name4cord_DSS[:10], '\n', len(scaf_name4cord_DSS)
##    print 'Done'
##    print 'UPS_DSS_Slicing completed'
...
##    print 'Done.\nTotal Entries processed for promoter prediction: ', total_loops
##    print 'promoter_prediction'
    return
...
def Processing_details(file_name, start_time, counter):
    ...
    print 'Total Promoter prediction attempted', total_loops
    end_time = time.time()
